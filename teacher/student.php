...
<html lang="zh_CN">
  ...
  <?php
    ...
    for($i=0;$rs=$res->fetch_assoc();$i++){
      ...
      $resw = $db->query($sqlw);
      $row[$i] = $resw->num_rows;
      $resql = "select * from weeknote where w_id='$uid' and w_isreply=1";
      $isreply = $db->query($resql);
      $replynum[$i] = $isreply->num_rows;
    }
  ?>
  <!-- 侧边导航-->
  <div role="navigation" id="foo" class="nav-collapse">
    ...
    <div role="main" class="main">
      ...
      <!-- 内容头部 -->
      <h2>实习学生周记信息<small class="pull-right">共<?= $sum?>个实习学生</small></h2>
      ...
      <tbody>
        <tr class="h5">
          ...
          <td width="15%">周记数/回复数</td>
        </tr>
        <?php
          for ($i=0; $i <$sum ; $i++) {
            echo "<tr>";
            echo "<td>".($i+1)."</td>"; 
            echo "<td><a href='weeknote.php?uid=".$u_student[$i]['u_id']."'>".$u_student[$i]['u_name']."</a></td>";
